...
(defmethod close ((server server) &key abort)
  (when (slot-boundp connection 'socket)
    (usocket:socket-close (socket connection))
    (slot-makunbound connection 'socket))
  (when abort
...
(defmethod serve ((root pathname) &rest args &key (timeout T))
  (remf args :timeout)
...
(defmethod fail ((client client) code message &rest args)
  (let ((packet (packet client)))
    ...
    (send client (nth-value 1 (setf (packet-error-message packet) (format NIL "~?" message args))))
    (close client)
...
(defmethod serve ((client client) &key (timeout (timeout client)))
  (cond ((usocket:wait-for-input (socket client) :timeout timeout)
         (let* ((packet (packet client))
...
(defmethod send ((client client) (file pathname) &key (filename (file-namestring file)) block-size timeout (synchronous T))
  (let ((packet (packet client)))
    (setf (packet-opcode packet) :wrq)
    (let ((start (nth-value 1 (setf (packet-strings packet) (list* filename "octet"))))
          (options (list :transfer-size (file-length file)
...
(defmethod receive ((client client) (file pathname) &key (filename (file-namestring file)) block-size (synchronous T))
  (let ((packet (packet client)))
...
(defmethod send ((hostname string) file &rest args &key (port 69))
  (remf args :port)
  (apply #'send (make-instance 'client :host hostname :port port) file args))

(defmethod receive ((hostname string) file &rest args &key (port 69))
  (remf args :port)
  (apply #'send (make-instance 'client :host hostname :port port) file args))
